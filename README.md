  <!-- Header Banner -->
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:4F8EF7,50:ff79c6,100:1a1b27&height=200&section=header&text=Muhammad%20Ali&fontSize=50&fontColor=ffffff&animation=fadeIn" width="100%" alt="Header Banner" />
  <!-- Animated Typing Header -->
  <a href="https://github.com/Alimemonnn">
    <img src="https://readme-typing-svg.demolab.com/?lines=AI+Student+%40+Aror+University;Full+Stack+Developer;Cybersecurity+%26+Open+Source+Enthusiast&font=Fira+Code&center=true&width=640&height=45&color=ff79c6&vCenter=true&pause=1000&size=24" alt="Typing Header" />
  </a>
  <!-- Social Badges -->
  <p align="center">
    <a href="https://www.linkedin.com/in/alimemonn/">
      <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" height="35" alt="LinkedIn">
    </a>&nbsp;
    <a href="https://x.com/alimemonnnn/">
      <img src="https://img.shields.io/badge/X-000000?style=for-the-badge&logo=x&logoColor=white" height="35" alt="X">
    </a>&nbsp;
    <a href="https://www.facebook.com/alimemonnn/">
      <img src="https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white" height="35" alt="Facebook">
    </a>&nbsp;
    <a href="https://www.instagram.com/ali_memonnn/">
      <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" height="35" alt="Instagram">
    </a>&nbsp;
    <a href="https://cyberaware-mauve.vercel.app/">
      <img src="https://img.shields.io/badge/Hackathon_Website-000000?style=for-the-badge&logo=vercel&logoColor=white" height="35" alt="CyberAware Hackathon Website">
    </a>
  </p>

  <!-- Quick Status Badges -->
  <p align="center">
    <img src="https://komarev.com/ghpvc/?username=Alimemonnn&label=Profile+Views&color=ff79c6&style=for-the-badge" alt="Profile Views" />
    &nbsp;
    <img src="https://img.shields.io/github/followers/Alimemonnn?label=Followers&style=for-the-badge&color=4F8EF7&logo=github" alt="Followers" />
    &nbsp;
    <img src="https://img.shields.io/badge/Status-Open_to_Work-brightgreen?style=for-the-badge&logo=statuspage&logoColor=white" alt="Open to Work" />
  </p>

</div>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/colored.png" width="100%" alt="Divider">

## 🧠 About Me

<img align="right" width="350" src="https://user-images.githubusercontent.com/74038190/212284158-e840e285-664b-44d7-b79b-e264b5e54825.gif" alt="Coding GIF">

Hi, I'm **Muhammad Ali** — an Artificial Intelligence student and aspiring Full Stack Developer. I love building practical, high-impact software that solves real-world problems, ranging from interactive web apps to desktop tools and AI systems.

- 🎓 **AI Student** at Aror University of Art, Architecture, Design & Heritage
- 🌱 Leveling up in **React**, **Spring Boot**, **Python**, and **AI / ML**
- 🔐 Passionate about **Cybersecurity** and digital safety research
- 🚀 Currently building: **[Steam Game Finder](https://github.com/Alimemonnn/Steam_Game_Finder_Software)** — a live Steam storefront clone & catalog evaluator
- 🤝 Open to **internships**, open-source collaborations, and tech opportunities
- 📬 Let's connect on any of the social channels above!

<br clear="right"/>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/colored.png" width="100%" alt="Divider">

## 🚀 Featured Projects

<div align="center">

| 🏆 Project | 📋 Description | 🛠️ Tech Stack | 🔗 Links |
| :---: | :--- | :---: | :---: |
| **CyberAware** | Interactive cybersecurity awareness platform built during a hackathon. Teaches threat detection, phishing prevention, and digital safety. | `HTML` `CSS` `JS` `Python` | [![Live](https://img.shields.io/badge/Live-4F8EF7?style=flat-square&logo=vercel&logoColor=white)](https://cyberaware-mauve.vercel.app) [![Repo](https://img.shields.io/badge/Repo-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Alimemonnn/CyberAware) |
| **Steam Game Finder** | Real-time Steam storefront clone querying live Steam APIs, presenting price/rating analytics, shortlist building, and CSV exports. | `Python` `Flask` `Steam API` | [![Live](https://img.shields.io/badge/Live-4F8EF7?style=flat-square&logo=vercel&logoColor=white)](https://steam-game-finder-software.vercel.app) [![Repo](https://img.shields.io/badge/Repo-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Alimemonnn/Steam_Game_Finder_Software) |
| **Website Blocker** | System-level productivity tool that blocks distracting sites via hosts file manipulation, featuring SHA-256 password security and Tkinter UI. | `Python` `Tkinter` `SHA-256` | [![Repo](https://img.shields.io/badge/Repo-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Alimemonnn/Website-Blocker-System) |
| **Campus Bite** | Home-cooked food platform tailored for university & hostel students. Built as an academic project focused on OOP principles and modular UI. | `Java` `OOP` `GUI` | [![Repo](https://img.shields.io/badge/Repo-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Alimemonnn/Campus_Bite) |
| **University Management** | Console-based application for managing student and staff records with persistent storage handling. | `C++` `CSV` `File I/O` | `Console App` |

</div>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/colored.png" width="100%" alt="Divider">

## 🛠️ Tech Stack & Skills

<p align="center">
  <b>Languages & Core</b><br>
  <img src="https://skillicons.dev/icons?i=py,js,ts,java,cpp,c,html,css" alt="Languages" />
</p>

<p align="center">
  <b>Frameworks & AI Libraries</b><br>
  <img src="https://skillicons.dev/icons?i=react,spring,flutter,flask,tensorflow,pytorch" alt="Frameworks" />
</p>

<p align="center">
  <b>Developer Tools & Platforms</b><br>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,idea,pycharm,postman,linux,ubuntu,mysql" alt="Tools" />
</p>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/colored.png" width="100%" alt="Divider">
<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/colored.png" width="100%" alt="Divider">

## 🏆 Badges & Achievements

<p align="center">
  <img src="https://img.shields.io/badge/Open_Source-Contributor-ff79c6?style=for-the-badge&logo=github&logoColor=white" alt="Contributor" />
  &nbsp;
  <img src="https://img.shields.io/badge/Developer-Full_Stack-4F8EF7?style=for-the-badge&logo=codeforces&logoColor=white" alt="Full Stack" />
  &nbsp;
  <img src="https://img.shields.io/badge/Focus-AI_%26_Security-brightgreen?style=for-the-badge&logo=python&logoColor=white" alt="AI & Security" />
</p>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/colored.png" width="100%" alt="Divider">

## 🐍 Contribution Snake

<p align="center">
  <img src="https://raw.githubusercontent.com/Alimemonnn/Alimemonnn/snake/github-snake-dark.svg" width="100%" alt="Contribution Snake">
</p>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/colored.png" width="100%" alt="Divider">

<div align="center">

  <!-- Dynamic Developer Quote -->
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight&border=true" width="100%" alt="Random Dev Quote" />

  <br><br>

  <!-- Terminal Card Art -->
```diff
+@ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @+
@@        o o                                           @@
@@        | |                                           @@
@@       _L_L_                                          @@
@@    ❮\/__-__\/❯  A habit missed once is a mistake,    @@
@@    ❮(|~o.o~|)❯    A habit missed twice is a start    @@
@@    ❮/ `-'/ \❯          of a new habit!               @@
@@      _/`U'\_                                         @@
@@     ( .   . )     .----------------------------.     @@
@@    / /     \ \    | while( ! (succeed=try() ) ) |     @@
@@    \ |  ,  | /    '----------------------------'     @@
@@     \|=====|/                                        @@
@@      |_.^._|                                         @@
@@      | |"| |                                         @@
@@      ( ) ( )   Testing leads to failure              @@
@@      |_| |_|   and failure leads to understanding    @@
@@  _.-' _j L_ '-._                                     @@
@@ (___.'     '.___)                                    @@
+@ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @+
